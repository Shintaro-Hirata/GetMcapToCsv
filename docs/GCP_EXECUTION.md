...
|--------|------|------------------------------|--------|
| vCPU + メモリ | 起動中のみ | 約 ¥25〜30/時 | 停止すれば ¥0。Spot なら約 1/3 |
...
3. マシンを大きくする（`MACHINE_TYPE=e2-standard-8` 等）— 抽出本体が並列で速くなる。
   時間単価は倍だが実行時間が短くなるため、1 回あたりの費用はほぼ変わらない。
...
| 項目 | 発生タイミング | 目安 | 備考 |
|------|----------------|------|------|
| VM の計算資源 | VM 起動中のみ | ¥25〜30/時（Spot なら約 1/3） | 10 分の抽出で ¥5 前後 |
