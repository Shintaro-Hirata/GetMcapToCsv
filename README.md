...
実行した条件は毎回自動保存され、タブの再読み込みや PC スリープで画面が初期状態に
戻っても、ページ上部の「♻ 前回の条件の自動保存」からワンクリックで復元できる。
VM 経由の抽出ジョブは UI と独立したプロセスで動くため、画面が初期状態に戻っても
「⏳ 未回収の VM 抽出ジョブ」から再接続してログ・CSV を回収できる。
