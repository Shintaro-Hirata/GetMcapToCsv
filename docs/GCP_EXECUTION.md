...
```
東京リージョンに VM（既定 e2-highcpu-32 = 32 vCPU / ディスク 200GB / バケット読み取り権限つき）を作り、
...
| コスト | いつ | 目安（既定 e2-highcpu-32 / 東京） | 抑え方 |
|--------|------|------------------------------|--------|
| vCPU + メモリ | 起動中のみ | 約 ¥150〜180/時（実行時間は 8vCPU の約 1/4 なので 1 回あたりはほぼ同じ） | 停止すれば ¥0。Spot なら約 1/3 |
| **ブートディスク** | **VM が存在する限り常時**（停止中も） | 200GB≒¥3,000/月（既定）· 30GB≒¥450/月 | モデル B なら存在時間が数十分で数円。モデル A は BOOT_DISK_GB を小さく |
...
- gcp.env に `BOOT_DISK_GB=30` を書いて作成する（既定は 200GB で停止中も月約 ¥3,000 かかるため、VM を残すモデル A では小さくするのがよい。ディスクが小さい分、抽出並列は空き容量に応じて自動で絞られる）。30GB なら停止中のディスク代は**約 ¥450/月**。
...
3. **マシンサイズを上げる（重い抽出で最も効く）** — 抽出はファイル単位の並列で
   vCPU-1 まで自動スケールする（上限 32）。record_sensor を含む数十 GB 級の抽出では
   デコードが CPU ネックになる。**既定を e2-highcpu-32（並列 31）にしてある**ので通常は
   そのままでよい。時間単価は上がるが実行時間がほぼ反比例で縮むため、**1 回あたりの費用は
   小さいマシンとほぼ同じ**。軽い抽出しかしないなら gcp.env で `MACHINE_TYPE=e2-standard-8`
   等に下げてもよい。
4. **バッチ実行の 2 件目以降はツール転送を自動スキップ** — UI の⑤バッチ実行は
   2 件目以降に `--skip-push` を自動で付け、ツール・mcap-ros2idl-support の再転送
   （30 秒〜1 分/件）を省く。手動実行でも `-SkipPush`（bash: `--skip-push`）を
   付ければ同じことができる（前回の実行で転送済みであることが前提）。
...
UI からも同じ点検ができる: ④ の「🔍 課金状況を確認」ボタンで上記と同じ内容を表示し、
VM が残っていた場合は **「🗑 残っている VM を削除する」ボタンでその場で削除**できる
（VM 経由ルートを開いたときにも残存を自動点検して警告する）。

さらに保険として、`gcp_create_vm` は既定で **起動したまま 12 時間たつと GCP 側が VM を
自動削除するタイマー**（`--max-run-duration` + `--instance-termination-action=DELETE`。
時間は直近の起動からの稼働時間で、停止すればリセット）を付けて作成する。ネットワーク断・
PC クラッシュでツール側の削除処理が一切動けなかった場合でも、課金はこの時間で必ず止まる。
長時間のバッチを回す場合や無効にしたい場合は gcp.env の `MAX_RUN_HOURS` で調整する
（`0` で無効。古い gcloud や組織ポリシーでこのフラグが使えない環境では、自動でタイマー
無しにフォールバックして作成する）。モデル A の停止中 VM は削除されないが、
**起動しっぱなしで 12 時間を超えると（停止ではなく）削除**される点だけ注意
（VM を確実に残したい場合は `MAX_RUN_HOURS=0` で作る）。
...
| 項目 | 発生タイミング | 目安 | 備考 |
|------|----------------|------|------|
| VM の計算資源 | VM 起動中のみ | 約 ¥150〜180/時（既定 e2-highcpu-32。Spot なら約 1/3） | 実行が速い分、10 分級の抽出で ¥10〜30 |
