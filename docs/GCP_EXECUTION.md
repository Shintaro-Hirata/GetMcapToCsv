...
3. **マシンサイズを上げる（重い抽出で最も効く）** — 抽出はファイル単位の並列で
   ...
   デコードが CPU ネックになるため、gcp.env に `MACHINE_TYPE=e2-highcpu-32` を書くと
   既定 (e2-standard-8, 並列 7) の**約 4 倍**で処理できる。時間単価は上がるが実行時間が
   ほぼ反比例で縮むため、**1 回あたりの費用はほぼ同じ**（短時間で終わるぶん得なことも多い）。
   逆に軽い抽出しかしないなら `MACHINE_TYPE=e2-standard-4` に下げてもよい。
